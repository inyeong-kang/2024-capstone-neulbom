...
// Check if the neck is stretched correctly
const checkNeckStretch = (landmarks) => {
  const leftEar = landmarks[7];
  const rightEar = landmarks[8];
  const neckBendThreshold = 0.2; // Threshold for neck bending

  // Check if head is tilted right
  if (rightEar.y - leftEar.y > neckBendThreshold) {
    return true;
  }
  return false;
};
...
recognition.onerror = (event) => {
  ...
  if (event.error === "aborted") {
    isRecognizing = false;
    document.getElementById("speech-toggle").innerText = "Start Speech";
  }
};
...
const API_KEY =
  "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";
...
function downloadTextFile() {
  ...
  link.download = "speech_data.txt"; // 다운로드할 파일 이름 설정
  link.click();
}
  ...
  const predict = async () => {
    ...
    if (poseResult.landmarks && poseResult.landmarks.length > 0) {
      for (const landmarks of poseResult.landmarks) {
        ...
        /*
        if (checkNeckStretch(landmarks)) {
          showStretchFeedback("Neck Stretching good!");
        } else if (checkShoulderStretch(landmarks)) {
          showStretchFeedback("Neck Stretching good!");
